...
define('YUNJIFEN_URL', 'http://9yc.com/');
//define('YUNJIFEN_URL', 'http://192.168.1.104:2222/');
...
*/
function buildCreditAutoLoginRequest($appKey,$appSecret,$uid,$credits,$extraParams=null){
    $url = YUNJIFEN_URL."autoLogin/autologin?";
    $timestamp=time()*1000 . "";
...
function buildCreditOrderStatusRequest($appKey,$appSecret,$orderNum,$bizId){
    $url=YUNJIFEN_URL."status/orderStatus?";
    $timestamp=time()*1000 . "";
...
/*
*  兑换订单审核请求
*  有些兑换请求可能需要进行审核，开发者可以通过此API接口来进行批量审核，也可以通过名天积分商城后台界面来进行审核处理
*/
function buildCreditAuditRequest($appKey,$appSecret,$passOrderNums,$rejectOrderNums){
    $url=YUNJIFEN_URL."audit/apiAudit?";
    $timestamp=time()*1000 . "";
...
/*
*  积分消耗请求的解析方法
*  当用户进行兑换时，名天积分商城会发起积分扣除请求，开发者收到请求后，可以通过此方法进行签名验证与解析，然后返回相应的格式
*  返回格式为：
...
/*
*  兑换订单的结果通知请求的解析方法
*  当兑换订单成功时，名天积分商城会发送请求通知开发者，兑换订单的结果为成功或者失败，如果为失败，开发者需要将积分返还给用户
*/
...
function parseRequest($appKey,$appSecret,$request_array) {
    if($request_array["appKey"] != $appKey){
        return false;
    }
    if($request_array["timestamp"] == null ){
...
function buildVirtualConfirmRequest($appKey, $appSecret, $orderNum, $success, $errorMessage, $credits){
    $url = YUNJIFEN_URL."confirm/confirm?";
    $timestamp=time()*1000 . "";
    $array=array("appKey"=>$appKey, "appSecret"=>$appSecret, "orderNum"=>$orderNum, "success"=>$success, "errorMsg"=>$errorMessage, "credits"=>$credits, "timestamp"=>$timestamp);
    $sign=sign($array);
    $url = $url . "appKey=" . $appKey . "&orderNum=" . $orderNum . "&success=" . $success . "&errorMsg=" . $errorMessage. "&credits=" . $credits . "&timestamp=" . $timestamp . "&sign=" . $sign;
    return $url;
}
?>