...
$(function (window) {
  ...
  function bangding(datas) {
      ...
      shopData.bazaar_name = datas.bazaar_name;
      $(".area-name").find(".info>p").text(shopData.bazaar_name);

      shopData.bazaar_phone = datas.bazaar_phone;
      $(".area_id_numbers").find(".info>p").text(shopData.bazaar_phone);
  ...
  // if (a === 0) {
  if (sid !== null || sid === "") {
    api('getAreaManagerInfo',{"sid": sid}).then(function(data){
        data = JSON.parse(data);
        if (data.status.succeed === 1) {
            var datas = data.data;
            bangding(datas);
        } else {
            swal({button: '关闭', text:`获取市场经理信息失败,${data.status.error_desc}`, type: 'error' });
        }
    }).catch(function(){
        swal({button: '关闭', text:`获取市场经理信息失败，请稍后重试！`});
    });
  } else {
