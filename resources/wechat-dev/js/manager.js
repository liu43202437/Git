...
$(function () {
  var peoData = {
    ...
    "id_number": "", //身份证号
     "manager_name": "", // 客户经理姓名
    // "area_managerid": "", //市场经理身份证号
    // "area_managername": "", // 市场经理姓名
    "area_id": "",
    "city":"",
    "address":"",
    'area_code':'',
  ...
  function areamanagerValidation() {
    if (!regName.test($areamanager.val())) {
      ...
      peoData.area_managername = $areamanager.val();
      return true;
    }
  }
  ...
  function areaidNoValidation() {
    if (!regIdNos.test($areaidNo.val())) {
      ...
      peoData.area_managerid = $areaidNo.val();
      return true;
    }
  }
  ...
  //   var listData = [{id:'1',value:'暂无人员信息'}];
  ...
  //     if (peoData.area_managerid) {
  //       var index =  listData.findIndex(function(item){
  //         return item.id === peoData.area_managerid;
  //       })
  //       if (index > -1) {
  ...
  //     peoData.area_managername = selectedItem[0].name;
  //     peoData.area_managerid = selectedItem[0].id;
  //     return true;
  //   }else {
  //     peoData.area_managername = '';
  //     peoData.area_managerid = '';
  //     return false;
  //   }
  // }
  ...
  $msgsBtn.click(function () {
    if ( peoData.phoneCode) {
      ...
      apiType({type:'POST',timeout: 5000},'ajaxSend',{"mobile": peoData.phone}).then(function(data){
        data = JSON.parse(data);
        if (data.success) {
        } else {
          swal({button:'关闭', text:`${data.error}`, type: 'error'});
          $msgsBtn.text('获取验证码');
          window.clearInterval(timer);
          timer = null;
          $msgsBtn.prop('disabled', false);
        ...
        swal({button:'关闭', text:`网络连接异常，请稍后重试`, type: 'error'});
        $msgsBtn.text('获取验证码');
        window.clearInterval(timer);
        timer = null;
        $msgsBtn.prop('disabled', false);
      ...
      apiByObj(reqData).then(function (data) {
        ...
        if (data.status.succeed === 1) {
          swal({
            ...
            Router.navigate('/resources/wechat/managerinfo.html', {
              sid: peoData.sid
            });
          });
        } else {
