Vue.component('visiter-zc', {
    ...
    <div>
      <div v-for="item in peoData">
        ...
        <div v-if="item.type === 'select'">
          <mobile-select :info="item.info" :list="item.list" @selectChange="selectChange($event,item)"></mobile-select>
        </div>
  ...
  created: function() {
    var refuse = axiosUrlParams('refuse');
      ...
      if( this.bindData(this.peoData,reqData)){
        ...
        .then(res=>{
          if (res.status.succeed === 1) {
            ...
            .then(()=>{
              setTimeout(function () {
                Router.navigateWithSid('/resources/wechat/vc-visiter-manager-info.html',{});
              }, 1000);
            });
          } else {
